            ...
            <div class="p-6">
                ...
                <div class="mt-6 grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <v-select multiple v-model="form.roles" :options="roles" label="name" appendToBody></v-select>
                ...
                <div class="mt-6 space-y-4">
                    <div class="space-y-2 border-2 border-dashed border-warning p-2">
                        ...
                        <div class="flex gap-2">
                            <span v-for="role in targetUser?.roles" :key="role.id" class="badge badge-info">{{ role.name }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 border-2 border-dashed border-warning p-2">
                        ...
                        <div v-if="targetUser?.permissions.length" class="flex flex-wrap gap-2">
                            <span v-for="permission in targetUser?.permissions" :key="permission.id" @click="removePermission(permission)" class="group badge hover:badge-error gap-2 cursor-pointer">
                    ...
                    <div  v-if="targetUser?.unassigned_permissions.length" class="space-y-2 border-2 border-dashed p-2">
                        <p class="font-bold">Available permissions for this user</p>
                        <div class="flex flex-wrap gap-2">
                            <span v-for="permission in targetUser?.unassigned_permissions" :key="permission.id" @click="addPermission(permission)" class="group badge hover:badge-success cursor-pointer gap-2">
...
const removePermission = (permission: Permission) => {
    try {
        let url = route("user.remove-permission", { user: targetUser.value?.id });
        router.delete(url,
        {
            data: {
                permission: permission.name

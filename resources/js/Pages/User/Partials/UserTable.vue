        ...
        <tbody>
          <tr v-for="user in users" :key="user.id">
            ...
            <td class="px-4 py-3 text-sm">
              {{ dayjs(user.created_at).format('HH:mm A, YYYY-MM-DD') }}
            </td>
            <td class="px-4 py-3 text-sm">
              {{ dayjs(user.updated_at).format('HH:mm A, YYYY-MM-DD') }}
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center space-x-4">
                <div class="tooltip" data-tip="View permissions">
                  <SecondaryButton aria-label="Permissions" @click="details(user)">
                    <ShieldCheckIcon class="w-5 h-5" />
                  </SecondaryButton>
                </div>
                <div class="tooltip" data-tip="Edit user">
                  <SecondaryButton aria-label="Edit" @click="edit(user)">
                    <PencilIcon class="w-5 h-5" />
                  </SecondaryButton>
                </div>
                <div class="tooltip" data-tip="Delete user">
                  <SecondaryButton aria-label="Delete" @click="remove(user)">
